...
CREATE TABLE IF NOT EXISTS log_analysis_history (
    ...
    -- 인시던트 연결
    incident_id      INTEGER REFERENCES incidents(id) ON DELETE SET NULL,
    -- 예외 처리 (alert_exclusions.id 연결, NULL = 정상 분석)
...
INSERT INTO chat_tools (name, display_name, description, input_schema, executor) VALUES
    ...
    ('admin_get_incident_context', '인시던트 컨텍스트 조회',
     ...
     '{"type":"object","properties":{"incident_id":{"type":"integer","description":"조회할 인시던트 ID. 화면 컨텍스트의 incident_id를 그대로 사용."}},"required":["incident_id"]}'::jsonb, 'admin'),
    ('admin_get_log_error_stats', '로그 실에러 건수 조회',
     ...
     '{"type":"object","properties":{"system_name":{"type":"string","description":"시스템명(system_name 또는 display_name). 예: cxm, 고객경험시스템"},"hours":{"type":"integer","description":"최근 N시간 기준 (기본 24)","default":24}},"required":["system_name"]}'::jsonb, 'admin'),
    ('admin_create_feedback', '인시던트 피드백 등록',
