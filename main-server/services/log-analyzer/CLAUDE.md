...
- **마커 형식:** `[이미지: {ocr_text}]` — DOCX/PPTX/PDF 통일 (Confluence는 alt/filename 사용)
...
- **단독 시각 마스킹 (2026-07-10 추가)**: 날짜 없는 `HH:MM:SS`(밀리초 `,644`/`.972` 포함) → `<TS>`. 스케줄 배치가 같은 메시지를 시각 슬롯마다 다른 template으로 갈라놓는 것 방지. 배경: dev-itenm OrderCloseService 3분 배치의 로그 선두 시각(`08:36:00 ERROR …`)이 기존 규칙(`YYYY-MM-DD HH:MM:SS` 전체 형식만 매칭)을 통과 → **동일 메시지 2종이 940개 포인트로 분열**, 각 포인트가 독립된 알림성 결정을 가져 사용자가 일부만 정보화하면 나머지 변형이 tier-1 stored-wins로 실에러 고착(2026-07-10 dev-itenm 35078/35079 사례). `HH:MM` 2단 형태(logback `Interpreter@12:18` 라인:칼럼)는 과병합 금지로 대상 제외.
- **값-마스킹 (2026-07-06 확장)**: KEY=VALUE 덤프/JSON body의 데이터값 마스킹 — ① 문자+숫자 혼합 할당값(`CUST_ID=C15292692`, `hnlounge4`, hex 해시)→`=<VAL>`, ② 한글·`?`(agent가 비ASCII 치환) 포함 할당값(고객명·매장명)→`=<VAL>`, ③ JSON 값 마스킹: 숫자·한글 포함 문자열→`<VAL>`, 숫자(`"loungeId":122`)→`<N>`, 빈 문자열→`<VAL>`, ④ 할당값 플레이스홀더·빈값 통일(`=<N>`/`=<NUM>`/`=`→`=<VAL>`), ⑤ **정규화 결과 400자 상한** — 리스트형 덤프(dcOrderDtlList)의 반복 `{…}` 블록 수(주문 품목 수)·agent 최대길이 절단 꼬리가 만드는 구조 변형 제거. **순수 문자 상태값(`=Y`/`=N`/`=FAILED`, `"successOrNot":"Y"`)은 보존** — 성공/실패 응답이 다른 template으로 유지. 배경: cxm ordInfo/dcCustInfo/dcOrderDtlList 덤프의 고객ID·이름·품목수가 매번 새 template을 만들어 **무한 카디널리티**(주기당 distinct 653, tier-1 영구 미스, 240s 타임아웃) 유발. 실측(운영 5분 창): **653→79 (-88%)** — 잔여는 유한한 Y/N 플래그 조합 (tier-2가 흡수).
- 과병합 금지: 공백 분리 단독 에러코드(`code 404`)는 묶지 않음. **`ORA-NNNNN`(Oracle 에러 코드)은 오류 유형 식별자라 `\d{5,}` 마스킹에서 제외**(2026-07-14, agent `template.rs`도 동일 예외 — 마스킹되면 어떤 유형인지 구분 불가). 규칙 추가 시 실데이터로 distinct 감소·비병합 동시 검증(`tests/test_normalize.py`).
