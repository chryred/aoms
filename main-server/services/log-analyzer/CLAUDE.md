...
> 전체 아키텍처·데이터 흐름·ADR 상세는 `.claude/memory/` 참조 (예: `.claude/memory/adrs.md`의 ADR-001 LLM Strategy, ADR-003 임베딩 교체, ADR-004 컬렉션 자동 보증).
...
- **임베딩**: Ollama `paraphrase-multilingual` (Server B) — 768차원, 2-core CPU 최적화 (ADR-003)
...
| `EMBED_MODEL` | `paraphrase-multilingual` (기존 bge-m3에서 교체, ADR-003) |
...
- `metric_hourly_patterns` / `aggregation_summaries`: `POST /aggregation/collections/setup` (또는 WF12) — 수동 1회
...
- `model_used` 필드에 `LLM_TYPE` 값 자동 기록 (devx/ollama/claude/openai)
