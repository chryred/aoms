...
async def _get_metric_context(system_id: int, system_name: str) -> str:
    """
    활성 collector_config가 있는 시스템에 한해 Prometheus instant query 결과를 반환.
    없거나 실패 시 빈 문자열 반환 → 기존 분석 방식 유지.
    """
    try:
        snapshot = await prom_client.get_metric_snapshot(system_name, system_id)
        return prom_client.format_metric_context(snapshot)
    except Exception as e:
        logger.debug(f"메트릭 컨텍스트 조회 실패 ({system_name}): {e}")
        return ""
...
async def analyze_with_vector_context(
    ...
    # 5. 강화 프롬프트 구성 + LLM 호출
    #    수집기가 등록된 시스템: 현재 메트릭 컨텍스트를 프롬프트에 추가
    prompt = build_enhanced_prompt(log_text, system_name, instance_role, anomaly_info)
    if system_id is not None:
        metric_ctx = await _get_metric_context(system_id, system_name)
        if metric_ctx:
            prompt = prompt + "\n\n" + metric_ctx
