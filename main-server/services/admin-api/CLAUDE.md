...
- 도구 그룹:
  ...
  - `qdrant` (ADR-011 RAG): 검색(Search) 6종 + 전문 조회(Get-Full) 3종.
    ...
    - 전문 조회 (검색에서 일부 청크만 받았을 때 LLM이 능동 호출): `qdrant_get_guide_full(guide_id)` / `qdrant_get_document_full(file_hash)` / `qdrant_get_confluence_full(page_id)`. 각각 청킹된 컬렉션(`knowledge_guides`, `knowledge_documents`, `knowledge_confluence_pages`)에서 chunk_index 순서로 전체 청크 반환. log-analyzer `GET /guides/{id}/chunks`, `GET /knowledge/documents/{hash}/chunks`, `GET /knowledge/confluence/{id}/chunks` 프록시
  ...
  - `prometheus`: `prometheus_query` — 보관 기간(운영 15d / 개발 3d) 이내 raw 메트릭 instant 조회. `system_name` + `metric_group(cpu|memory|disk|network|log|web|db)` + 선택 `time(KST)` / `window` / `aggregation(avg|max|min|p95|sum)`. 결과는 `instance_role`별 분리. 보관 기간 초과 입력은 자동 에러로 EMS/Qdrant 폴백 유도. 구현: `services/chat_tools/executors/prometheus.py`. 환경변수: `PROMETHEUS_URL`, `PROMETHEUS_RETENTION_DAYS`(기본 15)
