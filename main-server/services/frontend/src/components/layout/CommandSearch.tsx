    ...
    <div ref={containerRef} className="relative">
      <div className={cn(
        ...
        <input
          ...
          className={cn(
            'bg-transparent text-[#E2E8F2] placeholder-[#5A6478] outline-none text-sm',
        ...
        {/* 단축키 힌트 — 비포커스 시만 표시 */}
        {!focused && (
          <kbd className="hidden md:inline-flex items-center gap-0.5 shrink-0 text-[10px] text-[#3D4350] font-mono select-none">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-64 z-50 rounded-sm border border-[#2B2F37] bg-[#1E2127] shadow-[3px_3px_7px_#111317,-3px_-3px_7px_#2B2F37] overflow-hidden">
          {filtered.length === 0 ? (
            <p className="px-3 py-3 text-xs text-[#5A6478] text-center">결과 없음</p>
          ) : (
            <ul role="listbox">
              {filtered.map((item, idx) => {
                ...
                return (
                  <li key={item.to}>
                    {showGroup && (
                      <div className="px-3 pt-2 pb-1 text-[10px] text-[#5A6478] uppercase tracking-[0.08em]">
                        {item.group}
                      </div>
                    )}
                    <button
