  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-1">
            {(['all', 'critical', 'warning', 'normal'] as const).map((status) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all duration-150 min-h-[32px] focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:outline-none ${
                  filterStatus === status
                    ? status === 'all'
                      ? 'bg-[#A8B5C3] text-[#1A202C] shadow-md focus-visible:ring-[#A8B5C3]/50'
                      : status === 'critical'
                        ? 'bg-red-500/20 text-red-400 border border-red-500/30 shadow-md focus-visible:ring-red-500/50'
                        : status === 'warning'
                          ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 shadow-md focus-visible:ring-yellow-500/50'
                          : 'bg-green-500/20 text-green-400 border border-green-500/30 shadow-md focus-visible:ring-green-500/50'
      ...
      {filteredSystems.length === 0 ? (
        <div className="text-center py-8 text-[#8B97AD]">
