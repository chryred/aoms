...
export const EnhancedSystemCard = memo(function EnhancedSystemCard({
  ...
  const handleClick = () => {
    navigate(`${ROUTES.SYSTEMS}/${system.system_id}`)
  }
  ...
  return (
    ...
    >
      <NeuCard
        className={cn(
          'relative overflow-hidden cursor-pointer h-full transition-all duration-200',
        ...
        <div className="relative flex flex-col gap-3">
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              ...
              <h3 className="font-semibold text-[#E2E8F2] text-sm sm:text-base leading-snug break-words line-clamp-2">
                {system.display_name}
              </h3>
              <p className="font-mono text-xs text-[#8B97AD] truncate mt-0.5">
                {system.system_name}
              </p>
            </div>
            <ChevronRight className="h-4 w-4 text-[#8B97AD] flex-shrink-0 mt-1 transition-all duration-200 group-hover:translate-x-1 group-hover:text-[#00D4FF]" />
          </div>

          {/* 사유: 한줄 코멘트 */}
          <div className="px-3 py-2.5 rounded-md bg-[#1F2937]/40 border border-[#2A3447]/50">
            <p className="text-xs text-[#A8B5C3] leading-relaxed line-clamp-3 break-words">
              {system.reason || '상태 정보 수집 중...'}
            </p>
          </div>

          {/* 하단: 시스템 타입 + OS + 예방 패턴 뱃지 */}
          <div className="flex items-center gap-1.5 text-xs text-[#8B97AD] flex-wrap">
            <span className="px-2 py-0.5 rounded-sm bg-[#2A3447]/50 whitespace-nowrap">
              {system.system_type.toUpperCase()}
            </span>
            <span className="px-2 py-0.5 rounded-sm bg-[#2A3447]/50 capitalize whitespace-nowrap">
              {system.os_type}
            </span>
            {system.proactive_count > 0 && (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-sm bg-purple-500/10 text-purple-400 whitespace-nowrap">
