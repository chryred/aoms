...
export const contactsApi = {
  getContacts: () => adminApi.get('api/v1/contacts').json<Contact[]>(),

  getContact: (id: number) => adminApi.get(`api/v1/contacts/${id}`).json<Contact>(),
  ...
  deleteContact: (id: number) => adminApi.delete(`api/v1/contacts/${id}`),
