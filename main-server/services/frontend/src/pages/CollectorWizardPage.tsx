    ...
    <div className="max-w-2xl mx-auto">
      ...
      <NeuCard>
        ...
        {step === 1 && (
          ...
          >
            <p className="text-sm text-[#4A5568] mb-4">수집기 타입을 선택하세요</p>
        ...
        {step === 4 && (
          ...
          >
            ...
            {/* Info banner */}
            <div className="rounded-xl bg-[rgba(99,102,241,0.06)] border border-[rgba(99,102,241,0.2)]
                            px-4 py-3 text-sm text-[#4338CA] mb-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {customConfig.trim() === '' && (
                <p className="text-xs text-[#4A5568]">설정이 필요 없으면 비워두세요</p>
              )}
              {jsonError && (
                <p className="text-xs text-[#DC2626]">올바른 JSON 형식이 아닙니다: {jsonError}</p>
              )}
              {customConfig.trim() !== '' && !jsonError && (
                <p className="text-xs text-[#16A34A]">유효한 JSON입니다</p>
              )}
            </div>
          </WizardStepLayout>
        )}
        ...
        {step === 5 && (
          <WizardStepLayout onPrev={handlePrev}>
            <p className="text-sm text-[#4A5568] mb-4">입력한 내용을 확인하고 저장하세요</p>
            <NeuCard className="mb-4">
              <div className="flex flex-col gap-3 text-sm">
                <SummaryRow label="수집기 타입">
                  <span className="font-medium text-[#6366F1]">{collectorType}</span>
                </SummaryRow>
                ...
                <SummaryRow label="Prometheus Job">
                  <span className={prometheusJob ? 'text-[#1A1F2E]' : 'text-[#A0AEC0]'}>
                    {prometheusJob || '미설정'}
                  </span>
                </SummaryRow>
                <SummaryRow label="고급 설정">
                  {!customConfig.trim() ? (
                    <span className="text-[#A0AEC0]">없음</span>
                  ) : (
                    <div>
                      <pre
                        className={`text-xs font-mono text-[#1A1F2E] whitespace-pre-wrap overflow-hidden
      ...
      {showCancelDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/30" onClick={() => setShowCancelDialog(false)} />
          <div className="relative bg-[#E8EBF0] rounded-2xl p-6 shadow-xl max-w-sm w-full mx-4">
            ...
            <p className="text-sm text-[#4A5568] mb-4">입력 중인 내용이 초기화됩니다.</p>
...
function SummaryRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-4">
      <span className="w-32 flex-shrink-0 text-[#4A5568] font-medium">{label}</span>
