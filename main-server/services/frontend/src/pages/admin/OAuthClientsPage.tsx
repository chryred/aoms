    ...
    return (
      <div className="space-y-4">
        <div className="border-warning/30 bg-warning/5 rounded-sm border p-4 text-sm">
          ...
          <p className="text-text-secondary mt-1 text-xs">반드시 지금 저장하세요. 이후에는 재확인이 불가능합니다.</p>
        </div>

        <div className="space-y-2 text-sm">
          <div className="flex items-center justify-between rounded-sm border border-border bg-bg-base px-3 py-2">
        ...
        <NeuButton className="w-full" onClick={onClose}>닫기</NeuButton>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <NeuInput
            ...
            className="flex-1"
          />
          <NeuButton type="button" onClick={addUri} className="shrink-0 px-3">추가</NeuButton>
        </div>
        {uris.length > 0 && (
          <ul className="space-y-1">
            {uris.map((uri) => (
              <li key={uri} className="flex items-center justify-between rounded-sm border border-border bg-bg-base px-3 py-1.5 text-xs">
                <span className="text-text-primary font-mono">{uri}</span>
                <button type="button" onClick={() => removeUri(uri)} className="text-text-disabled hover:text-critical ml-2">
      ...
      <div className="flex gap-2 pt-2">
        ...
        <NeuButton variant="secondary" className="flex-1" onClick={onClose}>취소</NeuButton>
      </div>
    </div>
  )
}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-surface">
                <th className="px-4 py-3 text-left text-xs font-medium text-text-secondary">시스템 이름</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-secondary">Client ID</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-secondary">Redirect URI</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-secondary">상태</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-secondary">등록일</th>
                <th className="px-4 py-3" />
              </tr>
            </thead>
            <tbody>
              {clients.map((client) => (
                <tr key={client.id} className="border-b border-border last:border-0 hover:bg-surface/50 transition-colors">
                  <td className="px-4 py-3 font-medium text-text-primary">{client.name}</td>
                  <td className="px-4 py-3 font-mono text-xs text-text-secondary">{client.client_id}</td>
                  <td className="px-4 py-3">
                    <div className="space-y-0.5">
                      {(client.redirect_uris ?? []).map((uri) => (
                        <div key={uri} className="font-mono text-xs text-text-secondary">{uri}</div>
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-3">
